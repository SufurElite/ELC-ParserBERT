...
def is_caption(item):
    return (
        ...
        and item.attrib["type"].startswith("caption:")
    )
    ...
    def process_root(part, f, prefix="", suffix="", subchapter=None):
        for i, child in enumerate(part):
            ...
            process_written_part(
                child, f, prefix=prefix, suffix=suffix, subchapter=subchapter
            )
    ...
    def process_speech(part, f, prefix="", suffix="", subchapter=None):
        ...
        for i, child in enumerate(part):
            if child.tag == "speaker":
                ...
                process_paragraph(
                    child, f, prefix=new_prefix, suffix=suffix, subchapter=subchapter
                )
        ...
        for i, child in enumerate(part):
            if child.tag == "head":
                ...
                process_paragraph(
                    child,
                    f,
                    prefix="- " + (label + " " if label else "") + prefix,
                    suffix=suffix,
                    subchapter=subchapter,
                )
                label = None
