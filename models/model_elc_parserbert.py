...
# the above license is included in reference to the code associated with the ParserNetwork below
# which is largely taken or adapted from https://github.com/google-research/google-research/tree/master/structformer
...
class ParserBERT(nn.Module):
    def __init__(self, config, pad=0, activation_checkpointing=False):
        super().__init__()
        self.embedding = Embedding(config)
        self.parser_network = ParserNetwork(config, pad=pad)
    ...
    def forward(self, hidden_states, attention_mask, relative_embedding):
        ...
        for i in range(len(self.layers)):
            if self.activation_checkpointing:
                ...
                hidden_states.append(
                    self.layers[i](hidden_states, attention_mask[i], relative_embedding)
                )
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, relative_embedding):
        ...
        # shape: [B, H, Tq, Tk]
        # equivalent to attention_output_weights in structformer but scaled
        ...
        # we replace the attention mask with the one produced by structformer
        # i.e. the probabilitiy i attends to j, so we have Attention = p_{i}_{j}*q_{i}_{j}V_{j}
        # where q_{i}_{j} = sigmoid((QK^t)/(sqrt(dim_head)))

        # attention_mask shape should be:
        # [B, H, Tq, Tk]
